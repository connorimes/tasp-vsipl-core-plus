...
**********************************************************************/
/* $Id: VI_fftm_building_blocks_f.h,v 2.3 2007/04/16 16:33:39 judd Exp $ */

#ifndef _VI_FFTM_BUILDING_BLOCKS_F_H 
#define _VI_FFTM_BUILDING_BLOCKS_F_H
...
static
void 
VI_sortm_copy_f(void* tfft){
  vsip_fftm_f* fft = (vsip_fftm_f*) tfft;
...
static
void 
VI_init_fftm_f(void *TX,void* tfft){
  vsip_cmview_f *X = (vsip_cmview_f*)TX;
  vsip_fftm_f* fft = (vsip_fftm_f*) tfft;
...
static
void 
VI_ccfftmip_f(const vsip_fftm_f* fft, const vsip_cmview_f *X)
{
  void *fft_v = (void*)fft;
  void *X_v = (void*)X;
  VI_init_fftm_f(X_v,fft_v);
  if(fft->dft == 1){
     VI_dftm_f(fft_v);
  }else{
     VI_p0pFm_f(fft_v);
     VI_sortm_copy_f(fft_v );
  }
  if (fft->scale != 1){ 
     vsip_cvview_f A0 = *fft->x;
     vsip_cvview_f *x = &A0;
...
#endif /* _VI_FFTM_BUILDING_BLOCKS_F_H */
