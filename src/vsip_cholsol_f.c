...
**********************************************************************/
/* $Id: vsip_cholsol_f.c,v 2.2 2004/09/22 02:25:39 judd Exp $ */
...
void
VI_back_spd_subt_f(
          const vsip_mview_f *A,     /* VSIP_TR_LOW; size M by M */
          const vsip_mview_f *B)     /* In/out; size M by K                */
{
   vsip_scalar_f dot;
   ...
   vsip_scalar_f d0 = *(A->block->array + off_A);
   vsip_length n = n_B;
   ...
   vsip_scalar_f *a_re, *b;
   vsip_scalar_f *b_re = B->block->array + off_Br;
   while(n-- >0){
      *b_re /= d0;
       b_re += str;
   }
...
void
VI_back_spd_sub_f(
          const vsip_mview_f *A,    /* VSIP_TR_UPP, size M by M */
          const vsip_mview_f *B)    /* In/out; size M by K                          */
{
   vsip_scalar_f dot;
   vsip_scalar_f *b;
...
int
vsip_cholsol_f(
          const vsip_chol_f *chol,
          const vsip_mview_f *XB)
{
   int retval = 0;
   if(chol->uplo == VSIP_TR_UPP){
      VI_fwd_spd_subt_f(chol->matrix,XB);
      VI_back_spd_sub_f(chol->matrix,XB);
   } else { /* must be VSIP_TR_LOW */
      VI_fwd_spd_sub_f(chol->matrix,XB);
      VI_back_spd_subt_f(chol->matrix,XB);
   }
   return retval;
}  
