...
**********************************************************************/
/* $Id: vsip_cmexpoavg_d.c,v 2.0 2003/02/22 15:18:42 judd Exp $ */
 ...
 { 
    ...
    if(b == r){
       while(n_mn-- > 0){ /* in place */
          int n = n_mj;
          while(n-- >0){
            *rp_r = alpha * *rp_r + beta * *rp_r;
            *rp_i = alpha * *rp_i + beta * *rp_i;
       ...
       while(n_mn-- > 0){
          int n = n_mj;
          while(n-- >0){
            *rp_r = alpha * *bp_r + beta * *rp_r;
            *rp_i = alpha * *bp_i + beta * *rp_i;
