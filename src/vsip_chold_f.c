...
**********************************************************************/
/* $Id: vsip_chold_f.c,v 2.3 2004/09/22 02:25:39 judd Exp $ */
          ...
          const vsip_mview_f *A)
{
   ...
   for(k=0; k<n; k++){
      vsip_scalar_f *a_kk_r   = A->block->array + off_A_diag;
        ...
        const vsip_mview_f *A)
{
   ...
   vsip_scalar_f *a_kk_r = A->block->array + a_kk_off;

   vsip_stride A_diag_str = A->block->rstride * (A->row_stride + A->col_stride); 

   vsip_scalar_f re_scale;
          ...
          const vsip_mview_f *A)
{
   ...
   if(chol->uplo == VSIP_TR_LOW){
      retval = VI_cholesky_low_f(A);
   } else { /* must be vsip_tr_upp */
      retval = VI_cholesky_upp_f(A);
   } 
   return retval;
}
