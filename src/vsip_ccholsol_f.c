...
void
VI_cback_spd_sub_f(
          const vsip_cmview_f *A,    /* VSIP_TR_UPP, size M by M */
          const vsip_cmview_f *B)    /* In/out; size M by K                          */
{
   vsip_scalar_f dot_r,dot_i;
   vsip_scalar_f *b;
          ...
          const vsip_cmview_f *XB)
{
   ...
   if(chol->uplo == VSIP_TR_UPP){
      VI_cfwd_spd_subh_f(chol->matrix,XB);
      VI_cback_spd_sub_f(chol->matrix,XB);
   } else { /* must be VSIP_TR_LOW */
      VI_cfwd_spd_sub_f(chol->matrix,XB);
      VI_cback_spd_subh_f(chol->matrix,XB);
   }
   return retval;
}  
