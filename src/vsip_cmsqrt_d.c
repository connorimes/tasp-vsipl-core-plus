...
**********************************************************************/
/* $Id: vsip_cmsqrt_d.c,v 2.0 2003/02/22 15:18:44 judd Exp $ */
    ...
    if(ap_i == rp_i){ /* inplace */
       while(n_mn-- > 0){
         ...
         while(n-- >0){
           if (0.0 == *rp_i) {
              if(*rp_r < 0.0) {
                 *rp_i = (vsip_scalar_d)sqrt(-*rp_r);
                 *rp_r = 0.0;
              } else {
                 *rp_r = (vsip_scalar_d)sqrt(*rp_r);
              }
           } else {
             if (0.0 == *rp_r) {
                if(*rp_i > 0.0) {
                   *rp_i = (vsip_scalar_d)sqrt(0.5 * *rp_i);
                   *rp_r = *rp_i;
                } else {
                   *rp_i = (vsip_scalar_d)sqrt(0.5 * -*rp_i);
                   *rp_r = - *rp_i; }
                  ...
                  vsip_scalar_d mag = (vsip_scalar_d)sqrt(0.5 *
                     ((vsip_scalar_d)sqrt(*rp_r * *rp_r + *rp_i * *rp_i)
                     + ((*rp_r > 0) ? *rp_r : -*rp_r)));
                vsip_scalar_d s   = *rp_i/(vsip_scalar_d)( 2.0 * mag);
                if(*rp_r < 0.0) {
                   if(*rp_i < 0.0) {
                      *rp_r = -s; *rp_i = - mag;
                   } else {
       ...
       while(n_mn-- > 0){
          ...
          while(n-- >0){
             if (0.0 == *ap_i) {
                if(*ap_r < 0.0) {
                   *rp_i = (vsip_scalar_d)sqrt(-*ap_r);
                   *rp_r = 0.0;
                } else {
                   *rp_r = (vsip_scalar_d)sqrt(*ap_r);
                   *rp_i = 0.0;
                }
             } else {
                if (0.0 == *ap_r) {
                   if(*ap_i > 0.0) {
                      *rp_i = (vsip_scalar_d)sqrt(0.5 * *ap_i);
                      *rp_r = *rp_i;
                   } else {
                      *rp_i = (vsip_scalar_d)sqrt(0.5 * -*ap_i);
                      *rp_r = - *rp_i; 
                   ...
                   vsip_scalar_d mag = (vsip_scalar_d)sqrt(0.5 *
                        ((vsip_scalar_d)sqrt(*ap_r * *ap_r + *ap_i * *ap_i)
                        + ((*ap_r > 0) ? *ap_r : -*ap_r)));
                   vsip_scalar_d s   = *ap_i/(vsip_scalar_d)( 2.0 * mag);
                   if(*ap_r < 0.0) {
                     if(*ap_i < 0.0) {
                        *rp_r = -s; *rp_i = - mag;
                     } else {
