...
**********************************************************************/
/* $Id: vsip_rcmmul_d.c,v 2.0 2003/02/22 15:19:03 judd Exp $ */
...
void (vsip_rcmmul_d)(
  ...
  { 
    ...
    vsip_stride ast_mj, ast_mn,
                bst_mj, bst_mn,
                rst_mj, rst_mn;
    vsip_scalar_d *ap_r = (a->block->array) + a->offset * a->block->rstride,
    ...
    /* end define */
    if(rp_r == ap_r){ /* in-place in real part of output */
       vsip_scalar_d temp;
       while(n_mn-- > 0){
          int n = n_mj;
          while(n-- >0){
