...
**********************************************************************/
/* $Id: cmrealimagview_f.h,v 2.0 2003/02/22 15:23:32 judd Exp $ */
static void cmrealimagview_f(){
   printf("********\nTEST cmrealimagview_f\n");
   {
      ...
      vsip_mview_f       *rv = vsip_mrealview_f(v);
      vsip_mview_f       *iv = vsip_mimagview_f(v);
      vsip_scalar_f chk = 0;
      int i,j;
      vsip_cscalar_f a;
      for(i=0; i< (int)m; i++){
      ...
      printf("check mrealview_f view \n"); fflush(stdout);
      for(i=0; i< (int)m; i++){
         for(j=0; j< (int)n; j++){
            a.r = vsip_mget_f(rv,i,j);
            a.r -= data_r[i][j]; 
            chk += a.r * a.r;
         }
      }
      (chk > .0001) ?    printf("error \n") :
                         printf("correct \n");
      printf("check mimagview_f view \n"); fflush(stdout);
      chk = 0;
      for(i=0; i< (int)m; i++){
         for(j=0; j< (int)n; j++){
            a.i = vsip_mget_f(iv,i,j);
            a.i -= data_i[i][j]; 
            chk += a.i * a.i;
         }
      }
