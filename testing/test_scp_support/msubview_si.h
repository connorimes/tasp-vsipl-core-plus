...
**********************************************************************/
/* $Id: msubview_si.h,v 2.0 2003/02/22 15:23:34 judd Exp $ */
static void msubview_si(){
   printf("********\nTEST msubview_si\n");
   {
      ...
      vsip_mview_si        *sv = vsip_msubview_si(v,si,sj,sm,sn);
      ...
      for(i=0; i< (int)sm; i++)
         for(j=0; j< (int)sn; j++)
            chk += abs(vsip_mget_si(sv,i,j) - data[si+i][sj+j]);
