...
**********************************************************************/
/* $Id: tsubview_f.h,v 2.0 2003/02/22 15:23:35 judd Exp $ */
static void tsubview_f(){
   printf("********\nTEST tsubview_f\n");
   {
      ...
      vsip_tview_f        *sv = vsip_tsubview_f(v,szi,syi,sxi,szl,syl,sxl);
      vsip_scalar_f chk = 0;
      int i,j,k;
      for(i=0; i< (int)zl; i++)
         for(j=0; j< (int)yl; j++)
      ...
      for(i=0; i< (int)szl; i++)
         for(j=0; j< (int)syl; j++)
            for(k=0; k< (int)sxl; k++)
               chk += fabs(vsip_tget_f(sv,i,j,k) - data[szi+i][syi+j][sxi+k]);
