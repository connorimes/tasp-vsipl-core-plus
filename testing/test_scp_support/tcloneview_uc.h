...
**********************************************************************/
/* $Id: tcloneview_uc.h,v 2.0 2003/02/22 15:23:35 judd Exp $ */
static void tcloneview_uc(){
   printf("********\nTEST tcloneview_uc\n");
   {
      ...
      vsip_tview_uc        *sv = vsip_tcloneview_uc(v);
      ...
      for(i=0; i< (int)zl; i++)
         for(j=0; j< (int)yl; j++)
            for(k=0; k< (int)xl; k++)
               chk += abs(vsip_tget_uc(sv,i,j,k) - data[i][j][k]);
