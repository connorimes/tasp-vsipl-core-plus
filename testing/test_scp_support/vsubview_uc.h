...
**********************************************************************/
/* $Id: vsubview_uc.h,v 2.0 2003/02/22 15:23:36 judd Exp $ */
static void vsubview_uc(){
   printf("********\nTEST vsubview_uc\n");
   {
      ...
      vsip_index          si = 2;
      vsip_length         sn = 5;
      vsip_vview_uc       *sv = vsip_vsubview_uc(v,si,sn);
      vsip_scalar_uc chk = 0;
      int i;
      for(i=0; i< (int)n; i++)
           vsip_vput_uc(v,i,data[i]);
      for(i=0; i< (int)sn; i++)
          chk += abs(vsip_vget_uc(sv,i) - data[si+i]);
      (chk != 0) ?    printf("error \n") :
                      printf("correct \n");
      fflush(stdout);
      vsip_vdestroy_uc(sv);
