...
**********************************************************************/
/* $Id: vcloneview_mi.h,v 2.0 2003/02/22 15:23:35 judd Exp $ */
static void vcloneview_mi(){
   printf("********\nTEST vcloneview_mi\n");
   {
      ...
      vsip_vview_mi       *sv = vsip_vcloneview_mi(v);
      int chk = 0;
      int i;
      for(i=0; i< (int)n; i++){
           vsip_scalar_mi a;
      ...
      for(i=0; i< (int)n; i++){
           ...
           int a_r = (int)(a.r - data_r[i]);
           int a_c = (int)(a.c - data_c[i]);
           chk += a_r * a_r + a_c * a_c;
      }
      (chk != 0) ?    printf("error \n") :
                      printf("correct \n");
