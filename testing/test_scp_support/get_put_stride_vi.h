...
**********************************************************************/
/* $Id: get_put_stride_vi.h,v 2.1 2007/04/18 17:05:56 judd Exp $ */
static void get_put_stride_vi(){
   printf("********\nTEST get_put_stride_vi\n");
   {
       ...
       printf("test vgetstride_vi\n"); fflush(stdout);
       {
          s = vsip_vgetstride_vi(v);
          (s == ivs) ? printf("stride correct\n") : 
                       printf("stride error \n");
          fflush(stdout);
       }       
       /* the next test requires the previous test to be correct */
       printf("test vputstride_vi\n"); fflush(stdout);
       {
          vsip_vputstride_vi(v,jvs);
          s = vsip_vgetstride_vi(v);
