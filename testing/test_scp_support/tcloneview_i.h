...
**********************************************************************/
/* $Id: tcloneview_i.h,v 2.0 2003/02/22 15:23:35 judd Exp $ */
static void tcloneview_i(){
   printf("********\nTEST tcloneview_i\n");
   {
      ...
      vsip_tview_i        *sv = vsip_tcloneview_i(v);
      vsip_scalar_i chk = 0;
      int i,j,k;
      for(i=0; i< (int)zl; i++)
         for(j=0; j< (int)yl; j++)
      ...
      for(i=0; i< (int)zl; i++)
         for(j=0; j< (int)yl; j++)
            for(k=0; k< (int)xl; k++)
               chk += abs(vsip_tget_i(sv,i,j,k) - data[i][j][k]);
