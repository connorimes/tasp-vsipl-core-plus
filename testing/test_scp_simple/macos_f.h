...
**********************************************************************/
/* $Id: macos_f.h,v 2.0 2003/02/22 15:23:23 judd Exp $ */
static void macos_f(){
   printf("\n*******\nTEST macos_f\n\n");
   { vsip_scalar_f ans[] = {0, M_PI/4.0, M_PI/2.0, M_PI, .75 * M_PI, M_PI/2.0};
     vsip_scalar_f data[] = {1.0000, 0.707107, 0.0000,-1.0000,-0.707107,0.0000};
     vsip_block_f  *block = vsip_blockbind_f(data,6,VSIP_MEM_NONE);
     vsip_block_f  *ans_block = vsip_blockbind_f(ans,6,VSIP_MEM_NONE);
     vsip_mview_f  *a = vsip_mbind_f(block,0,1,3,3,2);
     vsip_mview_f  *ansm = vsip_mbind_f(ans_block,0,1,3,3,2);
