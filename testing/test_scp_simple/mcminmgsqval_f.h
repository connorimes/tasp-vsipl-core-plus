...
**********************************************************************/
/* $Id: mcminmgsqval_f.h,v 2.0 2003/02/22 15:23:24 judd Exp $ */
static void mcminmgsqval_f(){
   printf("\n*******\nTEST mcminmgsqval_f\n\n");
   { vsip_scalar_f data_r[]  = {-1, 2, 0, -5, -6,   3.4, -3.4,  5.6, -.3};
     ...
     vsip_scalar_mi ind_ans = vsip_matindex(2,2);
     vsip_scalar_f ans = 0.18;
     vsip_scalar_f val;
     ...
     val = vsip_mcminmgsqval_f(a,&index);
     ...
     val = vsip_mcminmgsqval_f(b,&index);
     printf("val = vsip_mcminmgsqval_f(b,index)\n matrix b = \n");VU_cmprintm_f("8.6",b);
     printf("val = %f\n",val); 
     printf("index = (%ld, %ld)\n",vsip_colindex(index),vsip_rowindex(index));
     if(fabs(ans - val) > .0001)
        printf("value error\n");
