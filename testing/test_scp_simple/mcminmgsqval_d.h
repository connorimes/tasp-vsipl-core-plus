...
**********************************************************************/
/* $Id: mcminmgsqval_d.h,v 2.0 2003/02/22 15:23:24 judd Exp $ */
static void mcminmgsqval_d(){
   printf("\n*******\nTEST mcminmgsqval_d\n\n");
   { vsip_scalar_d data_r[]  = {-1, 2, 0, -5, -6,   3.4, -3.4,  5.6, -.3};
     ...
     val = vsip_mcminmgsqval_d(a,&index);
     ...
     val = vsip_mcminmgsqval_d(b,&index);
     printf("val = vsip_mcminmgsqval_d(b,index)\n matrix b = \n");VU_cmprintm_d("8.6",b);
     printf("val = %f\n",val); 
     printf("index = (%ld, %ld)\n",vsip_colindex(index),vsip_rowindex(index));
     if(fabs(ans - val) > .0001)
        printf("value error\n");
