...
**********************************************************************/
/* $Id: ccfftop_sc_f.h,v 2.1 2007/04/18 17:05:50 judd Exp $ */
...
static void ccfftop_sc_f()
{
   printf("*********\nTEST ccfftop forward\n");
   {
      ...
      int i;
      vsip_scalar_f enp,t;
      vsip_cblock_f *block = vsip_cblockcreate_f(MAX_POINTS*7,VSIP_MEM_NONE);
      ...
      for(i=0; i<4; i++){
        printf("vsip_fft_f *fft = vsip_ccfftop_create_f(np,1.0,VSIP_FFT_FWD,0,VSIP_ALG_SPACE)\n");
        printf("np = %4d\n",np[i]);
        ...
        { /* do fft */
          {
          vsip_fft_f *fft = vsip_ccfftop_create_f(np[i],1.0,VSIP_FFT_FWD,0,VSIP_ALG_SPACE);
