...
**********************************************************************/
/* $Id: rcfftop_d.h,v 2.1 2007/04/18 17:05:50 judd Exp $ */
...
static void rcfftop_d()
{
   printf("********\nTEST rcfftop\n");
   {
      ...
      for(i=0; i<4; i++){
        printf("vsip_fft_d *fft = vsip_rcfftop_create_d(np,1.0/np,0,VSIP_ALG_SPACE)\n");
        printf("np = %4d\n",np[i]);
        ...
        { /* do fft */
          {
          vsip_fft_d *fft = vsip_rcfftop_create_d(np[i],1.0/np[i],0,VSIP_ALG_SPACE);
