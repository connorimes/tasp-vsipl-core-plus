...
**********************************************************************/
/* $Id: rcfftop_f.h,v 2.1 2007/04/18 17:05:50 judd Exp $ */
...
static void rcfftop_f()
{
   printf("********\nTEST rcfftop\n");
   {
      ...
      vsip_vview_f  *xnr = vsip_vrealview_f(xn);
      vsip_vview_f  *xni = vsip_vimagview_f(xn);
      ...
      for(i=0; i<4; i++){
        printf("vsip_fft_f *fft = vsip_rcfftop_create_f(np,1.0/np,0,VSIP_ALG_SPACE)\n");
        printf("np = %4d\n",np[i]);
        ...
        { /* do fft */
          {
          vsip_fft_f *fft = vsip_rcfftop_create_f(np[i],1.0/(vsip_scalar_f)np[i],0,VSIP_ALG_SPACE);
